...
public abstract class Uploader extends HttpServlet {
	 ...
	 */
	public void writeFile(DiskFileItem fi, File file) {
		try {
			...
			if (analyzer.isTextContent()) {
				...
				try {
					fi.write(Path.of(WebUtility.normalizePath(file.getAbsolutePath())));
				} catch (Exception e) {
	...
	/**
	 * Take a character stream and convert it to a String
	 * 
	 * @param reader
	 * @return
	 * @throws IOException
