...
public class FileUploader extends Uploader {
	...
	private List<Map<String, String>> getBaseUploadData(List<DiskFileItem> fileItems, Insight in, String relativePath,
		...
		// Always validate the selected directory, including uploads without a subpath.
		...
		java.nio.file.Path destination = WebUtility.resolveWithin(assetRoot,
				containedRelativePath == null ? "" : containedRelativePath);
		Files.createDirectories(destination);
		// Recheck after creation so an existing symlink cannot redirect the upload.
		destination = WebUtility.resolveWithin(assetRoot,
				containedRelativePath == null ? "" : containedRelativePath);
	...
	private List<Map<String, String>> processFileItems(List<DiskFileItem> fileItems, String filePath, String fePath)
		...
		while (iteratorFileItems.hasNext()) {
			...
			if (!fi.isFormField()) {
				...
				writeFileWithin(fi, Paths.get(filePath), file.toPath());
