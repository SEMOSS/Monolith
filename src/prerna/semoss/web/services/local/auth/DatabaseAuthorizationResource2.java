...
public class DatabaseAuthorizationResource2 {
	...
	public Response getDatabases(@Context HttpServletRequest request,
			@QueryParam("databaseId") List<String> databaseFilter, @QueryParam("filterWord") String searchTerm,
...
//		}
	...
	public Response getUserDatabasePermission(@Context HttpServletRequest request,
		...
		if (permission == null) {
			classLogger.warn("User is trying to pull permission details for database {} without having proper access",
					databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
	...
	public Response getDatabaseUsers(@Context HttpServletRequest request, @QueryParam("databaseId") String databaseId,
		...
		try {
			...
			List<Map<String, Object>> members = SecurityEngineUtils.getEngineUsers(user, databaseId, searchParam,
			...
			classLogger.warn("User is trying to pull users for database {} without having proper access", databaseId);
	...
	public Response addDatabaseUserPermission(@Context HttpServletRequest request,
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to add users for database {} but is not an admin", databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
		...
		try {
			...
			classLogger.warn("User is trying to add users for database {} without having proper access", databaseId);
	...
	public Response addDatabaseUserPermissions(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
				"CALLING LEGACY ENDPOINT - NEED TO UPDATE TO GENERIC ENGINE ENDPOINT /auth/engine/addEngineUserPermissions with PARAM engineId");
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to add user permissions to database {} but is not an admin", databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		}
	...
	public Response editDatabaseUserPermission(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to edit user {} permissions for database {} but is not an admin",
		...
		try {
			...
			classLogger.warn("User is trying to edit user {} permissions for database {} without having proper access",
					existingUserId, databaseId);
	...
	public Response editDatabaseUserPermissions(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
				"CALLING LEGACY ENDPOINT - NEED TO UPDATE TO GENERIC ENGINE ENDPOINT /auth/engine/editEngineUserPermissions with PARAM engineId");
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to edit user permissions for database {} but is not an admin", databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		}
		...
		// log the operation
		classLogger.info("User has edited user permission to database {}", databaseId);
	...
	public Response removeDatabaseUserPermission(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to remove user {} from having access to database {} but is not an admin",
					existingUserId, databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		...
		try {
			...
			classLogger.warn(
					"User is trying to remove user {} from having access to database {} without having proper access",
					existingUserId, databaseId);
	...
	public Response removeDatabaseUserPermissions(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
				"CALLING LEGACY ENDPOINT - NEED TO UPDATE TO GENERIC ENGINE ENDPOINT /auth/engine/removeEngineUserPermissions with PARAM engineId");
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to remove users from having access to database {} but is not an admin",
					databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		...
		try {
			...
			classLogger.warn(
					"User is trying to remove users from having access to database {} without having proper access",
					databaseId);
	...
	public Response setDatabaseGlobal(@Context HttpServletRequest request, MultivaluedMap<String, String> form) {
		...
		if (AbstractSecurityUtils.adminOnlyEngineSetPublic(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to set the database {}{} but is not an admin", databaseId, logPublic);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		}
		...
		try {
			...
			classLogger.warn("User is trying to set the database {}{} without having proper access", databaseId,
					logPublic);
	...
	public Response setDatabaseDiscoverable(@Context HttpServletRequest request, MultivaluedMap<String, String> form) {
		...
		if (AbstractSecurityUtils.adminOnlyEngineSetPublic(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to set the database {}{} but is not an admin", databaseId,
					logDiscoverable);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "User session is invalid");
			return WebUtility.getResponse(errorMap, 401);
	...
	public Response setDatabaseVisibility(@Context HttpServletRequest request, MultivaluedMap<String, String> form) {
		...
		try {
			...
			classLogger.warn("User is trying to set the database {}{} without having proper access", databaseId,
					logVisible);
			classLogger.error("Failed to update database visibility.", e);
			Map<String, String> errorRet = new HashMap<String, String>();
			errorRet.put(Constants.ERROR_MESSAGE, e.getMessage());
			return WebUtility.getResponse(errorRet, 400);
		} catch (Exception e) {
		...
		// log the operation
		classLogger.info("User has set the database {} {}", databaseId, logVisible.trim());
	...
	public Response setDatabaseFavorite(@Context HttpServletRequest request, MultivaluedMap<String, String> form) {
		...
		try {
			...
			classLogger.warn("User is trying to set the database {}{} without having proper access", databaseId,
					logFavorited);
			classLogger.error("Failed to update database favorite.", e);
			Map<String, String> errorRet = new HashMap<String, String>();
			errorRet.put(Constants.ERROR_MESSAGE, e.getMessage());
			return WebUtility.getResponse(errorRet, 400);
		} catch (Exception e) {
		...
		// log the operation
		classLogger.info("User has set the database {} {}", databaseId, logFavorited.trim());
	...
	public Response getDatabaseUsersNoCredentials(@Context HttpServletRequest request,
			@QueryParam("databaseId") String databaseId, @QueryParam("searchTerm") String searchTerm,
	...
	public Response approveDatabaseUserAccessRequest(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
				"CALLING LEGACY ENDPOINT - NEED TO UPDATE TO GENERIC ENGINE ENDPOINT /auth/engine/approveEngineUserAccessRequest with PARAM engineId");
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to approve user access to database {} but is not an admin", databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		}
		...
		try {
			...
			classLogger.warn("User is trying to grant user access to database {} without having proper access",
					databaseId);
		...
		// log the operation
		classLogger.info("User has approved user access and added user permissions to database {}", databaseId);
	...
	public Response denyDatabaseUserAccessRequest(@Context HttpServletRequest request,
			MultivaluedMap<String, String> form) {
		classLogger.warn(
				"CALLING LEGACY ENDPOINT - NEED TO UPDATE TO GENERIC ENGINE ENDPOINT /auth/engine/denyEngineUserAccessRequest with PARAM engineId");
		...
		if (AbstractSecurityUtils.adminOnlyEngineAddAccess(databaseId) && !SecurityAdminUtils.userIsAdmin(user)) {
			classLogger.warn("User is trying to deny user access to database {} but is not an admin", databaseId);
			Map<String, String> errorMap = new HashMap<String, String>();
			errorMap.put(Constants.ERROR_MESSAGE, "This functionality is limited to only admins");
			return WebUtility.getResponse(errorMap, 401);
		}
