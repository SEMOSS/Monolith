...
public final class AnthropicMessagesHelper {
	...
	public static Map<String, Object> normalizeMessage(Map<String, Object> message, Room room, Insight insight) {
	 ...
	 * 
	 * IMPORTANT: Each Anthropic message must map to internal messages while
	 * preserving the message boundary so that when Python rebuilds messages for
	 * Anthropic, the user/assistant alternation is maintained.
	 * 
	 * Strategy: - User messages with text/image -> single INPUT_TEXT message (with
	 * mediaInputs if images present) - User messages with tool_result -> one
	 * INPUT_TOOL_EXEC per tool_result - Assistant messages with text -> single
	 * RESPONSE_TEXT message - Assistant messages with tool_use -> single
	 * RESPONSE_TOOL message (all tool_uses grouped)
	 */
	@SuppressWarnings("unchecked")
	public static List<Map<String, Object>> normalizeMessages(Object messages, Object systemPrompt, Room room,
			Insight insight) {
		...
		// 2. Process Message List - each Anthropic message becomes one or more internal
		// messages
...
// Add images as list of data URIs/URLs for later processing by copyFilesToRoomFolder
						if (!imageBlocks.isEmpty()) {
							List<String> imageDataUris = new ArrayList<>();
							for (Map<String, Object> imageBlock : imageBlocks) {
								String imageUri = extractImageAsDataUri(imageBlock);
							...
							userMsg.put("imageDataUris", imageDataUris);
						}
	...
	/**
	 * Upload image files to room Returns list of image files uploaded to the room
	 */
	private static List<String> uploadImagesToRoom(List<String> inputImages, Room room, Insight insight) {
		Path roomPath = Path.of(room.getRoomFolderPath());
		List<String> fileNames = new ArrayList<String>();
		for (String image : inputImages) {
			String fileName = MessageUtils.writeBase64ImageDataUriToDir(image, roomPath);
