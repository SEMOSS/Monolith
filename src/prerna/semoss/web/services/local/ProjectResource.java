...
public class ProjectResource {
	...
	public Response updateSmssFile(@Context HttpServletRequest request, @PathParam("projectId") String projectId) {
		...
		// like update the engine id / alias
		{
			...
			if (!newProp.get(Constants.PROJECT).equals(currentSmssProperties.get(Constants.PROJECT))
				...
				errorMap.put(Constants.ERROR_MESSAGE,
						"The project id, project name, and project type cannot be changed");
				return WebUtility.getResponse(errorMap, 400);
			}
		}
		...
		try {
			...
			Map<String, String> errorMap = new HashMap<>();
			errorMap.put(Constants.ERROR_MESSAGE,
					"An error occurred initializing the new project details. Detailed message = " + e.getMessage());
			return WebUtility.getResponse(errorMap, 400);
		}
	...
	@Produces("application/json;charset=utf-8")
	public Response runReactor(@Context HttpServletRequest request, @PathParam("projectId") String projectId,
		...
		try {
			...
			if (reactor == null) {
				throw new IllegalArgumentException(
						"Could not find custom reactor " + reactor + " in project " + projectId);
			}
...
//						.lastModified(new Date(file.lastModified()))
	...
	@Path("/embedLogo")
	@Produces({ MediaType.TEXT_HTML, MediaType.APPLICATION_OCTET_STREAM })
	public Response getEmbedUrl(@Context final Request coreRequest, @Context HttpServletRequest request,
			@PathParam("projectId") String projectId, @QueryParam("insightId") String insightId) {
...
//					.lastModified(new Date(exportFile.lastModified()))
	...
	public Response downloadInsightImage(@Context final Request coreRequest, @Context HttpServletRequest request,
		...
		File exportFile = getInsightImageFile(projectId, id, WebUtility.inputSanitizer(request.getHeader("Referer")),
				params, sessionId);
		if (exportFile != null && exportFile.exists()) {
			String exportName = projectId + "_Image." + FilenameUtils.getExtension(exportFile.getAbsolutePath());
			// want to cache this on browser if user has access
...
//			}
	...
	@Path("/jdbc")
	@Produces(MediaType.APPLICATION_OCTET_STREAM)
	public StreamingOutput getJDBCOutput(@PathParam("projectId") String projectId,
			@QueryParam("insightId") String insightId, @QueryParam("sql") String sql, @QueryParam("open") String open,
		...
		boolean firstTime = !projectId.equalsIgnoreCase("session") && (open != null && open.equalsIgnoreCase("true"));
	...
	public StreamingOutput getJDBCCSVOutput(@PathParam("projectId") String projectId,
		...
		if (!projectId.equalsIgnoreCase("session")) // && InsightStore.getInstance().get(insightId) == null) // see if
													// you can open the insight
		{
			NameServer server = resourceContext.getResource(NameServer.class);
			OverrideParametersServletRequest requestWrapper = new OverrideParametersServletRequest(request);
			Map<String, String> paramMap = new HashMap<String, String>();
