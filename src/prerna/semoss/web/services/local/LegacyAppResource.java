	...
	public Response updateSmssFile(@Context HttpServletRequest request, @PathParam("databaseId") String databaseId) {
		...
		try {
			...
			if (!isAdmin) {
				...
				if (!isOwner) {
					throw new IllegalAccessException("Database " + databaseId
		...
		try {
			...
			errorMap.put(Constants.ERROR_MESSAGE,
					"An error occurred reading the current database smss details. Detailed message = "
							+ e.getMessage());
			return WebUtility.getResponse(errorMap, 400);
		}
		try {
			...
			Map<String, String> errorMap = new HashMap<>();
			errorMap.put(Constants.ERROR_MESSAGE,
					"An error occurred closing the connection to the database. Detailed message = " + e.getMessage());
			return WebUtility.getResponse(errorMap, 400);
		}
		try {
			...
			Map<String, String> errorMap = new HashMap<>();
			errorMap.put(Constants.ERROR_MESSAGE,
					"An error occurred initializing the new database details. Detailed message = " + e.getMessage());
			return WebUtility.getResponse(errorMap, 400);
		}
...
//					.lastModified(new Date(exportFile.lastModified()))
	...
	protected File getDatabaseImageFile(String databaseId) throws Exception {
		...
		String fileLocation = EngineUtility.getSpecificEngineVersionFolder(IEngine.CATALOG_TYPE.DATABASE, databaseId,
				databaseName);
