 ...
 * 
 * Expected format: [[SEMOSS_CONTEXT:insightId=xxx,roomId=yyy]] The tag is
 * removed so it never reaches the downstream model.
 */
public class SemossContextExtractor {

	private static final Pattern CONTEXT_PATTERN = Pattern.compile("\\[\\[SEMOSS_CONTEXT:(.*?)\\]\\]\\s*");

	private static final Pattern PARENT_ROOM_PATTERN = Pattern.compile("\\[\\[PARENT_ROOM_ID=(.*?)\\]\\]\\s*");
	...
	/**
	 * Extract SEMOSS context from a raw string and return the cleaned string plus
	 * any extracted IDs.
	 */
	public static ExtractionResult extract(String text) {
		...
		return new ExtractionResult(kvMap.get("insightId"), kvMap.get("roomId"));
	}
	...
	/**
	 * Extract context from the latest message in the Anthropic messages list, and
	 * strip the tag from the message content in-place.
	 *
	 * Handles both simple string content: {"role": "user", "content":
	 * "[[SEMOSS_CONTEXT:...]] hello"}
	 *
	 * And structured content block arrays: {"role": "user", "content": [{"type":
	 * "text", "text": "[[SEMOSS_CONTEXT:...]] hello"}, ...]}
	 *
	...
	/**
	 * Extract the parent room ID from a [[PARENT_ROOM_ID=...]] tag in the system
	 * prompt.
	 *
