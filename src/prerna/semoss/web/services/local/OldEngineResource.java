...
//				String paramValueStr = coreEngine.getTransformedNodeName(paramValue[i], false);
				...
				if (coreEngine.getDatabaseType() == DATABASE_TYPE.RDBMS) {
					...
					if (paramValueCol != null) {
						query = query.replaceFirst(Pattern.quote(paramBind[i]), Matcher.quoteReplacement(paramValueCol));
					}
					if (paramValueTable != null) {
						query = query.replaceFirst(Pattern.quote(paramBind[i]), Matcher.quoteReplacement(paramValueTable));
					}

				} else {
					query = query.replaceFirst(Pattern.quote(paramBind[i]), Matcher.quoteReplacement(paramValueStr));
				}
			}
		}
